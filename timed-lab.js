...
const LAB_VERSION = "timed-platform-lab-v1";
...
const LAB_MANIFEST = {
  version: LAB_VERSION,
  mode: "timed-platform-lab",
  seed: "TIMED-PLATFORM-LAB",
  spawn: { ...INITIAL_SPAWN },
  pieces: [
    { id: "timed-start", assetId: "platform-square-blue", position: [0, 0, 8], colliders: PLATFORM_COLLIDERS },
    {
      id: "alternating-a",
      assetId: "platform-square-blue",
      position: [0, 0, 3.8],
      colliders: PLATFORM_COLLIDERS,
      timed: { solidDuration: 1250, warningDuration: 350, hiddenDuration: 900, phase: 0 }
    },
    {
      id: "alternating-b",
      assetId: "platform-square-blue",
      position: [0, 0, -0.4],
      colliders: PLATFORM_COLLIDERS,
      timed: { solidDuration: 1250, warningDuration: 350, hiddenDuration: 900, phase: 0.5 }
    },
    {
      id: "timed-checkpoint-one",
      assetId: "platform-square-blue",
      position: [0, 0, -4.6],
      colliders: PLATFORM_COLLIDERS,
      checkpoint: { id: "timed-checkpoint-1", label: "Alternating checkpoint", index: 1, spawn: [0, 0.32, -4.6] }
    },
    {
      id: "wave-a",
      assetId: "platform-square-blue",
      position: [0, 0, -8.8],
      colliders: PLATFORM_COLLIDERS,
      timed: { solidDuration: 1050, warningDuration: 300, hiddenDuration: 850, phase: 0 }
    },
    {
      id: "wave-b",
      assetId: "platform-square-blue",
      position: [0, 0, -13],
      colliders: PLATFORM_COLLIDERS,
      timed: { solidDuration: 1050, warningDuration: 300, hiddenDuration: 850, phase: 0.333 }
    },
    {
      id: "wave-c",
      assetId: "platform-square-blue",
      position: [0, 0, -17.2],
      colliders: PLATFORM_COLLIDERS,
      timed: { solidDuration: 1050, warningDuration: 300, hiddenDuration: 850, phase: 0.666 }
    },
    {
      id: "timed-checkpoint-two",
      assetId: "platform-square-blue",
      position: [0, 0, -21.4],
      colliders: PLATFORM_COLLIDERS,
      checkpoint: { id: "timed-checkpoint-2", label: "Wave checkpoint", index: 2, spawn: [0, 0.32, -21.4] }
    },
    {
      id: "fast-cycle",
      assetId: "platform-square-blue",
      position: [0, 0, -25.6],
      colliders: PLATFORM_COLLIDERS,
      timed: { solidDuration: 650, warningDuration: 220, hiddenDuration: 760, phase: 0.15 }
    },
    { id: "timed-finish-platform", assetId: "platform-square-blue", position: [0, 0, -29.8], colliders: PLATFORM_COLLIDERS },
    {
      id: "timed-finish-gate",
      assetId: "finish-wide",
      position: [0, 1, -30.9],
      scale: 0.45,
  ...
  bounds: { minX: -10, maxX: 10, minZ: -37, maxZ: 14, minHeight: -8, maxHeight: 12 },
  expectedColliderCount: 20,
  expectedTimedCount: 6,
  checkpointCount: 2
};
...
function createModel(parent, piece, asset) {
  ...
  fallback.setAttribute("color", piece.timed ? "#06B6D4" : "#2563EB");
...
function addTimedPlatform(pieceEntity, piece) {
  const settings = piece.timed;
  const warning = createEntity("a-ring");
  warning.setAttribute("data-timed-warning", piece.id);
  warning.setAttribute("position", "0 1.035 0");
  warning.setAttribute("rotation", "-90 0 0");
  warning.setAttribute("radius-inner", "1.2");
  warning.setAttribute("radius-outer", "1.55");
  warning.setAttribute("color", "#22D3EE");
  warning.setAttribute("material", "emissive: #0891B2; emissiveIntensity: 0.85; opacity: 0.9; transparent: true");
  warning.setAttribute("visible", false);
  pieceEntity.appendChild(warning);

  pieceEntity.setAttribute(
    "timed-platform",
    `rig: #player-rig; solidDuration: ${settings.solidDuration}; warningDuration: ${settings.warningDuration}; hiddenDuration: ${settings.hiddenDuration}; phase: ${settings.phase}`
  );
}

function addInstructionSigns(root) {
  const signs = [
    { position: [-3.4, 2.55, 3.8], rotation: "0 90 0", text: "1. READ THE ALTERNATING PAIR" },
    { position: [3.4, 2.55, -4.6], rotation: "0 -90 0", text: "2. FOLLOW THE THREE-PLATFORM WAVE" },
    { position: [-3.4, 2.55, -21.4], rotation: "0 90 0", text: "3. COMMIT TO THE FAST FINAL CYCLE" }
  ];
...
function registerTimedLabSafety() {
  if (!window.AFRAME || AFRAME.components["timed-lab-safety"]) return;
  AFRAME.registerComponent("timed-lab-safety", {
    schema: {
      minX: { default: -10 }, maxX: { default: 10 },
      minZ: { default: -37 }, maxZ: { default: 14 },
    ...
    tick: function (time) {
      ...
      if (invalid || outside) this.resetPlayer("Fall reset — returned to the latest timed-platform checkpoint");
    },
...
function buildLab() {
  ...
  for (const piece of LAB_MANIFEST.pieces) {
    ...
    if (piece.timed) addTimedPlatform(pieceEntity, piece);
  }

  addInstructionSigns(root);
  rig.setAttribute("platformer-surface-extension", "");
  rig.setAttribute("timed-lab-safety", "");
  ...
  window.dispatchEvent(new CustomEvent("course-built", {
    detail: {
      ...
      timedPlatformCount: root.querySelectorAll("[timed-platform]").length,
...
function setupLabLifecycle() {
  ...
  function safety() {
    return rig.components["timed-lab-safety"];
  }
  ...
  function restartLab() {
    ...
    setStatus("Lab restarted. Cyan rings warn just before a platform disappears.", "ready");
    details.textContent = `Checkpoint 0/2 • 6 timed platforms • ${LAB_VERSION}`;
    setWorld("Timed-platform lab ready");
    window.dispatchEvent(new CustomEvent("course-request-reset", {
      detail: { spawn: { ...INITIAL_SPAWN }, message: "Timed-platform lab restarted" }
    }));
  }
  ...
  async function preflight() {
    ...
    const timed = document.querySelectorAll("[timed-platform]");
    if (window.__LOCOMOTION_LOAD_FAILED__ || !AFRAME.components["gorilla-locomotion"]) problems.push("Gorilla locomotion did not load");
    if (!AFRAME.components["timed-platform"]) problems.push("timed-platform component is missing");
    ...
    if (timed.length !== LAB_MANIFEST.expectedTimedCount) problems.push(`expected ${LAB_MANIFEST.expectedTimedCount} timed platforms, found ${timed.length}`);
    ...
    note.textContent = "Timed-platform lab passed preflight. Enter VR and test all three timing patterns.";
    note.dataset.state = "ready";
    setStatus("Lab ready: alternating pair, three-platform wave, and fast final cycle.", "ready");
    setWorld("Enter VR to test timed platforms");
  ...
  function frame(now) {
    if (!state.startedAt && !state.completed && rig.object3D.position.z < 6.1) {
      state.startedAt = now;
      window.dispatchEvent(new CustomEvent("course-started", { detail: { startedAt: now, mode: "timed-platform-lab" } }));
    }
    if (state.startedAt && !state.completed && now - state.lastUi >= 120) {
      state.lastUi = now;
      details.textContent = `Time ${((now - state.startedAt) / 1000).toFixed(1)}s • Checkpoint ${state.checkpoint}/2 • State changes ${state.transitions}`;
    }
  ...
  window.addEventListener("timed-platform-state", (event) => {
    state.transitions += 1;
    const stateName = event.detail?.state || "unknown";
    const id = String(event.detail?.platformId || "timed platform").replaceAll("-", " ");
    if (stateName === "warning") setWorld(`${id.toUpperCase()} IS BLINKING`);
  });
  window.addEventListener("course-finish", () => {
    if (state.completed) return;
    state.completed = true;
    const elapsed = state.startedAt ? performance.now() - state.startedAt : 0;
    setStatus(`Timed-platform lab complete in ${(elapsed / 1000).toFixed(1)}s.`, "ready");
    details.textContent = `All 6 timed-platform tests completed • Checkpoints ${state.checkpoint}/2`;
    setWorld("TIMED-PLATFORM LAB COMPLETE");
  });

  leftHand?.addEventListener("controllerconnected", () => setWorld("Left controller tracked — waiting for right"));
  rightHand?.addEventListener("controllerconnected", () => setWorld("Controllers tracked — watch the cyan warnings"));
  ...
  window.requestAnimationFrame(frame);
  window.funFunTimedLab = { manifest: LAB_MANIFEST, state, restart: restartLab };
}

registerGeneratedComponents();
registerTimedLabSafety();
setupLabLifecycle();
buildLab();
